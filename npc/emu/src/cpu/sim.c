...
void runCPUSimModule(bool *inst_end_flag) {
    if (!sim_ebreak) {
        sim_pc   = top->io_ifuio_oPC;
        sim_snpc = sim_pc + 4;
        sim_inst = top->io_ifuio_oInst;

        runCPUSimModuleCycle();

        sim_dnpc = top->io_ifuio_oPC;
