...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra,    R, imm = BITS(src2, 5, 0); word_t bit_upper = BITS(src1, 63, 63); word_t bits = src1; for (int i = 0; i < imm; i++) { bits = bits >> 1; bits = (bit_upper << 63) | bits; } R(rd) = bits);
  INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai,   I, imm = BITS(imm, 5, 0); word_t bit_upper = BITS(src1, 63, 63); word_t bits = src1; for (int i = 0; i < imm; i++) { bits = bits >> 1; bits = (bit_upper << 63) | bits; } R(rd) = bits);
  ...
  INSTPAT("000000 ?????? ????? 101 ????? 01110 11", srlw,   R, R(rd) = SEXT(BITS(src1, 31, 0) >> BITS(src2, 4, 0), 32));
  INSTPAT("000000 ?????? ????? 101 ????? 00110 11", srliw,  I, R(rd) = SEXT(BITS(src1, 31, 0) >> BITS(imm, 5, 0), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw,   R, imm = BITS(src2, 4, 0); word_t bit_upper = BITS(src1, 31, 31); word_t bits = BITS(src1, 31, 0); for (int i = 0; i < imm; i++) { bits = bits >> 1; bits = (bit_upper << 31) | bits; } R(rd) = SEXT(bits, 32));
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw,  I, imm = BITS(imm, 5, 0); word_t bit_upper = BITS(src1, 31, 31); word_t bits = BITS(src1, 31, 0); for (int i = 0; i < imm; i++) { bits = bits >> 1; bits = (bit_upper << 31) | bits; } R(rd) = SEXT(bits, 32));
