...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra,    R, R(rd) = (sword_t)src1 >> (sword_t)BITS(src2, 5, 0));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai,   I, R(rd) = (sword_t)src1 >> (sword_t)BITS( imm, 5, 0));
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw,   R, R(rd) = SEXT(BITS(BITS(src1, 31, 0) <<  BITS(src2, 4, 0), 31, 0), 32));
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw,  I, R(rd) = SEXT(BITS(src1 << BITS(imm, 5, 0), 31, 0), 32));
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw,   R, R(rd) = SEXT(BITS(src1, 31, 0) >> BITS(src2, 4, 0), 32));
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw,  I, R(rd) = SEXT(BITS(src1, 31, 0) >> BITS( imm, 5, 0), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw,   R, R(rd) = SEXT((sword_t)src1 >> (sword_t)BITS(src2, 4, 0), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw,  I, R(rd) = SEXT((sword_t)src1 >> (sword_t)BITS( imm, 5, 0), 32));
  ...
  INSTPAT("??????? ????? ????? 100 ????? 01100 11", xor,    R, R(rd) = src1 ^ src2);
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret,   N, s->dnpc = SR(CSR_MEPC) + 4);
